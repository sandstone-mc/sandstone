...
export class BasePathClass<N extends (undefined | string) = (undefined | string), D extends (undefined | string) = (undefined | string)> {
  ...
  /** Create a tag. */
  Tag = <T extends TAG_TYPES>(type: T, name: string, values: TagSingleValue<HintedTagStringType<T> | TagInstance<T>>[] = [], options?: TagOptions<T>) => (
    new TagInstance(this.datapack, type, this.getName(name), values, { onConflict: this.onConflict?.tag, ...(options ?? {}) })
  )
