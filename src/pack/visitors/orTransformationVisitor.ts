...
import { AndNode, ConditionNode, IfNode, NotNode, OrNode } from '../../flow/index.js'
import { ContainerCommandNode, ContainerNode, Node } from '../../core/index.js'
...
export class OrTransformationVisitor extends GenericSandstoneVisitor {
  ...
  parseOrNode = (node: OrNode): { preNodes: Node[], conditionNode: ConditionNode} => {
    ...
    const realPreNodes: Node[] = [
      new ScoreboardCommandNode(this.pack, 'players', 'reset', variable),
    ]
    const orExecuteNodes = node.conditions.map((condition) => {
      ...
      ifNode.body = [
        new ScoreboardCommandNode(this.pack, 'players', 'set', variable, 1),
      ]
      return ifNode
    })
  ...
  parseAndNode = (node: AndNode): { preNodes: Node[], conditionNode: ConditionNode} => {
    ...
    const conditionNode = new AndNode(node.sandstoneCore, node.conditions.map((condition) => {
