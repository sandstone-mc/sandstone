import type { JsonSymbolSoundType } from 'sandstone/arguments/generated/_json/dispatcher.ts'
...
export type JsonSound = NonNullable<({
  [S in Extract<Extract<JsonSoundType, string>, string>]?: {
    /**
    ...
    name: (S extends undefined
      ? JsonSymbolSoundType<'%none'> :
      (S extends keyof JsonSymbolSoundType ? JsonSymbolSoundType[S] : JsonRootNBT)),
    /**
     * Defaults to 1.0.
     *
     * Value:
     ...
     */
    attenuation_distance?: (NBTInt | number),
  }
}[Extract<JsonSoundType, string>])>

export type JsonSoundEventRegistration = {
  /**
   ...
   */
  sounds?: Array<((JsonRegistry['minecraft:sound'] | SoundEventClass) | JsonSound)>,
  /**
   ...
   */
  subtitle?: JsonRegistry['minecraft:translation_key'],
}

export type JsonSounds = ({
  [Key in NonEmptyString]?: JsonSoundEventRegistration
})

export type JsonSoundType = ('file' | 'event')
