import { Set } from 'sandstone'
import type { NamespacedLiteralUnion, SetType } from 'sandstone'

export type JSONWORLDGEN_FEATURES = (
  | NamespacedLiteralUnion<SetType<typeof JSONWORLDGEN_FEATURES_SET>>
  | `minecraft:${SetType<typeof JSONWORLDGEN_FEATURES_SET>}`)

export const JSONWORLDGEN_FEATURES_SET = new Set([
  'acacia',
  'amethyst_geode',
  'azalea_tree',
  'bamboo_no_podzol',
  'bamboo_some_podzol',
  'bamboo_vegetation',
  'basalt_blobs',
  'basalt_pillar',
  'berry_bush',
  'birch',
  'birch_bees_0002',
  'birch_bees_0002_leaf_litter',
  'birch_bees_002',
  'birch_bees_005',
  'birch_leaf_litter',
  'birch_tall',
  'blackstone_blobs',
  'blue_ice',
  'bonus_chest',
  'brown_mushroom',
  'bush',
  'cactus',
  'cave_vine',
  'cave_vine_in_moss',
  'cherry',
  'cherry_bees_005',
  'chorus_plant',
  'clay_pool_with_dripleaves',
  'clay_with_dripleaves',
  'coral/block_decoration',
  'coral/brain_block',
  'coral/bubble_block',
  'coral/fire_block',
  'coral/horn_block',
  'coral/tube_block',
  'crimson_forest_vegetation',
  'crimson_fungus',
  'crimson_fungus_planted',
  'crimson_roots',
  'dark_forest_vegetation',
  'dark_oak',
  'dark_oak_leaf_litter',
  'dead_bush',
  'delta',
  'desert_well',
  'disk_clay',
  'disk_grass',
  'disk_gravel',
  'disk_sand',
  'dripleaf',
  'dripstone_cluster',
  'dry_grass',
  'end_gateway_delayed',
  'end_gateway_return',
  'end_island',
  'end_platform',
  'end_podium_active',
  'end_podium_inactive',
  'end_spike',
  'fallen_birch_tree',
  'fallen_jungle_tree',
  'fallen_oak_tree',
  'fallen_poplar_tree',
  'fallen_spruce_tree',
  'fallen_super_birch_tree',
  'fancy_oak',
  'fancy_oak_bees',
  'fancy_oak_bees_0002_leaf_litter',
  'fancy_oak_bees_002',
  'fancy_oak_bees_005',
  'fancy_oak_leaf_litter',
  'firefly_bush',
  'flower_cherry',
  'flower_default',
  'flower_flower_forest',
  'flower_meadow',
  'flower_pale_garden',
  'flower_plain',
  'flower_swamp',
  'forest_flowers',
  'forest_rock',
  'fossil_coal',
  'fossil_diamonds',
  'freeze_top_layer',
  'glow_lichen',
  'glowstone_extra',
  'grass',
  'grass_jungle',
  'huge_brown_mushroom',
  'huge_red_mushroom',
  'ice_patch',
  'ice_spike',
  'iceberg_blue',
  'iceberg_packed',
  'jungle_bush',
  'jungle_tree',
  'jungle_tree_no_vine',
  'kelp',
  'lake_lava',
  'large_basalt_columns',
  'large_dripstone',
  'large_fern',
  'leaf_litter',
  'lush_caves_clay',
  'mangrove',
  'mangrove_vegetation',
  'meadow_trees',
  'mega_jungle_tree',
  'mega_pine',
  'mega_spruce',
  'melon',
  'monster_room',
  'moss_patch',
  'moss_patch_bonemeal',
  'moss_patch_ceiling',
  'moss_vegetation',
  'mushroom_island_vegetation',
  'nether_sprouts',
  'nylium_bonemeal',
  'oak',
  'oak_bees_0002_leaf_litter',
  'oak_bees_002',
  'oak_bees_005',
  'oak_leaf_litter',
  'orange_poplar',
  'orange_poplar_leaf_litter',
  'ore_ancient_debris_large',
  'ore_ancient_debris_small',
  'ore_andesite',
  'ore_blackstone',
  'ore_clay',
  'ore_coal',
  'ore_coal_buried',
  'ore_copper_large',
  'ore_copper_small',
  'ore_diamond_buried',
  'ore_diamond_large',
  'ore_diamond_medium',
  'ore_diamond_small',
  'ore_diorite',
  'ore_dirt',
  'ore_emerald',
  'ore_gold',
  'ore_gold_buried',
  'ore_granite',
  'ore_gravel',
  'ore_gravel_nether',
  'ore_infested',
  'ore_iron',
  'ore_iron_small',
  'ore_lapis',
  'ore_lapis_buried',
  'ore_magma',
  'ore_nether_gold',
  'ore_quartz',
  'ore_redstone',
  'ore_soul_sand',
  'ore_tuff',
  'pale_forest_flower',
  'pale_garden_vegetation',
  'pale_moss_patch',
  'pale_moss_patch_bonemeal',
  'pale_moss_vegetation',
  'pale_oak',
  'pale_oak_bonemeal',
  'pale_oak_creaking',
  'patch_fire',
  'patch_soul_fire',
  'pile_hay',
  'pile_ice',
  'pile_melon',
  'pile_pumpkin',
  'pile_snow',
  'pine',
  'pointed_dripstone',
  'pumpkin',
  'red_mushroom',
  'red_poplar',
  'red_poplar_leaf_litter',
  'red_shrub',
  'rooted_azalea_tree',
  'rooted_sulfur_spring',
  'sculk_patch_ancient_city',
  'sculk_patch_deep_dark',
  'sculk_vein',
  'sea_pickle',
  'seagrass_mid',
  'seagrass_short',
  'seagrass_slightly_less_short',
  'seagrass_tall',
  'small_basalt_columns',
  'spore_blossom',
  'spring_lava_frozen',
  'spring_lava_nether',
  'spring_lava_overworld',
  'spring_nether_closed',
  'spring_nether_open',
  'spring_water',
  'spruce',
  'sugar_cane',
  'sulfur_pool',
  'sulfur_spike',
  'sulfur_spike_cluster',
  'sulfur_spring',
  'sunflower',
  'super_birch_bees',
  'super_birch_bees_0002',
  'swamp_oak',
  'taiga_grass',
  'tall_grass',
  'tall_mangrove',
  'trees_badlands',
  'trees_birch',
  'trees_birch_and_oak_leaf_litter',
  'trees_dappled_forest',
  'trees_flower_forest',
  'trees_grove',
  'trees_jungle',
  'trees_old_growth_pine_taiga',
  'trees_old_growth_spruce_taiga',
  'trees_plains',
  'trees_savanna',
  'trees_snowy',
  'trees_sparse_jungle',
  'trees_taiga',
  'trees_water',
  'trees_windswept_hills',
  'twisting_vines',
  'underwater_magma',
  'vines',
  'void_start_platform',
  'warm_ocean_vegetation',
  'warped_forest_vegetation',
  'warped_fungus',
  'warped_fungus_planted',
  'waterlily',
  'weeping_vines',
  'wildflower',
  'yellow_poplar',
  'yellow_poplar_leaf_litter',
] as const)
