...
export type ClickAction = ({
  [S in Extract<Registry['minecraft:dialog_action_type'], string>]?: ({
    type: S
  } & (S extends keyof Dispatcher<'minecraft:dialog_action'>
    ? Dispatcher<'minecraft:dialog_action'>[S]
...
export type DynamicCustomAction = ({
  [S in Extract<`${string}:${string}`, string>]?: {
    ...
    /**
         ...
         */
    additions?: (S extends keyof Dispatcher<'mcdoc:custom_dynamic_event_additions'>
      ? Dispatcher<'mcdoc:custom_dynamic_event_additions'>[S]
      : Record<string, unknown>)
  };
}[`${string}:${string}`])
