 ...
 */
type StringSmallerThan4<T extends string> = T extends `${infer _A}${infer _B}${infer _C}${infer _D}${infer _E}`
  ? ['ERROR: String must be 3 characters long, or smaller.', never]
  : T

type CharacterOfString<STRING extends string> = STRING extends `${infer A}${infer B}` ? A | CharacterOfString<B> : never

type CharacterOfStringsArray<ARRAY extends string[]> = ARRAY extends [infer A, ...infer B]
  ? A extends string
    ? B extends string[]
...
type RecipeKind<
  ...
  HAS_GROUP extends boolean = true,
> = {
  /**
...
type KeysIngredients<T extends [string, string, string]> = Record<
  Exclude<CharacterOfStringsArray<T>, ' '>,
  ObjectOrArray<ItemOrTag>
>
