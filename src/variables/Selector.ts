 ...
 */
type SelectorArguments<MustBeSingle extends boolean> = {
  /**
  ...
  /**
   * Filter target selection based on their rotation in the horizontal XZ-plane, measured clockwise in degrees from due south (or the positive Z direction). 
   * 
   * Values vary:
  ...
  /** Define a position on the X-axis in the world the selector starts at, for use with the `distance` argument or the volume arguments, `dx`, `dy` and `dz`. */
  x: number

  /** Define a position on the Y-axis in the world the selector starts at, for use with the `distance` argument or the volume arguments, `dx`, `dy` and `dz`. */
  y: number

  /** Define a position on the Z-axis in the world the selector starts at, for use with the `distance` argument or the volume arguments, `dx`, `dy` and `dz`. */
  z: number
}) & ({} | {
  /** Filter target selection based on their x-difference, from some point, as measured from the closest corner of the entities' hitboxes */
  dx: number

  /** Filter target selection based on their y-difference, from some point, as measured from the closest corner of the entities' hitboxes */
  dy: number

  /** Filter target selection based on their z-difference, from some point, as measured from the closest corner of the entities' hitboxes */
  dz: number
}) & (
    MustBeSingle extends true ? { limit: 0 | 1 } : { limit?: number }
  )

function isSingleSelector(target: LiteralUnion<'@s' | '@p' | '@a' | '@e' | '@r'>, selectorArguments: SelectorArguments<true> | SelectorArguments<false>): boolean {
  if (['@s', '@p', '@r'].includes(target)) {
    return true
  }

  const limit = selectorArguments.limit
...
export class SelectorClass<IsSingle extends boolean> {
  ...
  arguments: SelectorArguments<IsSingle>

  constructor(target: LiteralUnion<'@s' | '@p' | '@a' | '@e' | '@r'>, selectorArguments?: SelectorArguments<IsSingle>) {
    this.target = target

    this.arguments = selectorArguments ?? {} as SelectorArguments<IsSingle>
  }

  isSingle(): IsSingle {
    return isSingleSelector(this.target, this.arguments) as IsSingle
  }
  ...
  toString() {
    ...
    if (this.arguments) {
      ...
      const modifiers = {
        ...
        // Parse advancements
        advancements: (advancements: AdvancementsArgument) => result.push(['advancements', parseAdvancements(advancements)]),
        ...
        predicate: (predicate: string | string[]) => {
          ...
          result.push(...predicates.map((predicate) => ['predicate', predicate] as const))
        },
    ...
    return this.target + '[' + result.map(([key, value]) => { return `${key}=${value}` }).join(',') + ']'
  }
...
type AnySelectorArguments = SelectorArguments<false>
type SingleSelectorArguments = SelectorArguments<true>
type NoLimitSelectorArguments = Omit<SelectorArguments<false>, 'limit'>

export function Selector(target: '@s' | '@p' | '@r', selectorArguments?: NoLimitSelectorArguments): SelectorClass<true>
export function Selector(target: '@a' | '@e', selectorArguments: SingleSelectorArguments): SelectorClass<true>
export function Selector(target: '@a' | '@e', selectorArguments?: AnySelectorArguments): SelectorClass<false>
export function Selector(target: string, selectorArguments?: AnySelectorArguments): SelectorClass<false>
export function Selector<T extends boolean>(target: LiteralUnion<'@s' | '@p' | '@a' | '@e' | '@r'>, selectorArguments?: SelectorArguments<T>): SelectorClass<T> {
