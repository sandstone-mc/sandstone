...
/**
 * Gathers all generated function texts from the sandstonePack.
 * This function executes the provided callback to generate all resources,
 * then collects the text content of all MCFunction resources.
 *
 * @example
 ...
 * }
 *
 * @param callback - The function that will generate all resources.
 * @returns A promise that resolves to a record of generated function texts.
 */
export async function getGeneratedFunctionsText(callback: () => void): Promise<Record<string, string[]>> {
  sandstonePack.reset()
...
export async function compareOutputText(callback: () => void, expectedContents: Record<string, string[]>) {
  const accumulatedResults = await getGeneratedFunctionsText(callback)
...
export async function compareSingleOutputText(callback: () => void, expectedContent: string[]) {
  compareOutputText(
    () => {
      MCFunction('default:test', callback)
    },
    {
